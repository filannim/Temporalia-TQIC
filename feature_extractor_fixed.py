...
class FeatureExtractor(object):
	...
	def __timing(self, sentence, utterance):
		...
		2) numerical difference between query and utterance (in # of months)
		...
		if years and not months:					# compute year-distance
			...
			mean_utterance = ((utterance.year*12) + utterance.month)
	...
	def __get_trigger_classes(self, sentence):
		...
		if c:
			sorted_classes = sorted(c.items(), key=lambda e: (1/e[1],e[0]))
			return top(sorted_classes), '-'.join([k for k,c in sorted_classes])
		else:
			return '', ''
	...
	def __TempoWordNet_classes(self, sentence, tempowordnet, lemmatizer):
		...
		for k,c in scores.iteritems():
			scores[k] = c/total
			if scores[k] == 0: scores[k] = 0.00000001
		sorted_scores = sorted(scores.items(), key=lambda e: (1/e[1],e[0]))
		return top(sorted_scores), sorted_scores, scores
	...
	def __tenses(self, sentence):
		...
		sorted_postags = sorted(filtered_postags.items(), key=lambda e: (1/e[1],e[0]))
		return top(sorted_postags), '-'.join([k for k,c in sorted_postags])
	...
	def __pos_ordered_footprint(self, sentence):
		...
		sorted_postags = sorted(pos_tags.items(), key=lambda e: (1/e[1],e[0]))
		return top(sorted_postags), '-'.join([k for k,c in sorted_postags])
	...
	def extract(self, sentence, utterance):
		...
		first_tempowordnet_class, tempowordnet_sorted, tempowordnet_counter = self.__TempoWordNet_classes(sentence, self.tempowordnet, self.wordnetlemmatizer)
		tempowordnet_classes = '-'.join([k for k,c in tempowordnet_sorted])
		tempowordnet_past = str(tempowordnet_counter['past'])
		tempowordnet_present = str(tempowordnet_counter['present'])
		tempowordnet_future = str(tempowordnet_counter['future'])
		tempowordnet_atemporal = str(tempowordnet_counter['atemporal'])
		first_trigger_class, trigger_classes = self.__get_trigger_classes(sentence)
		trigger_past = 'T' if 'past' in trigger_classes.split('-') else 'F'
		trigger_present = 'T' if 'present' in trigger_classes.split('-') else 'F'
		trigger_future = 'T' if 'future' in trigger_classes.split('-') else 'F'
		trigger_atemporal = 'T' if 'atemporal' in trigger_classes.split('-') else 'F'
		...
		return {'QUERY':sentence.raw_text,
				...
				'tempowordnet_past': tempowordnet_past,
				'tempowordnet_present': tempowordnet_present,
				'tempowordnet_future': tempowordnet_future,
				'tempowordnet_atemporal': tempowordnet_atemporal,
				...
				'trigger_past': trigger_past,
				'trigger_present': trigger_present,
				'trigger_future': trigger_future,
				'trigger_atemporal': trigger_atemporal,
				'tenses': tenses,
